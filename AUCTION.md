...
- `src/pages/Settings/PlayersTable.tsx` (pre-draft rankings table, distinct from the during-draft `PlayersLeftTab` in [SNAKE.md](SNAKE.md) Gap #3) — computes `isAuction` once and threads it into `PlayerRow.tsx`/`PlayerRowMobile.tsx`: auction shows `$`/`vs. market` (`StandardValueLabel.tsx`); snake/linear shows `ADP` (Sleeper ADP averaged with ESPN's overall rank, ESPN alone for superflex - see `blendedAdpByFpid`) and `vs ADP` (`AdpValueLabel.tsx` - this app's own cross-position rank, built by sorting every active-position player by `dollarValue` purely as an internal ranking key, never displayed - minus blended ADP). Tier column is unchanged either way (already format-agnostic).
