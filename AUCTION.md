...
- **`convex/sleeper/draftSync.ts`** (whole file) + `convex/draft/picks.ts`'s **`applySleeperSyncedPicks`** — live sync from an in-progress Sleeper draft (auction, snake, or linear) into `draftPicks`, polling on a self-rescheduling `internalAction` chain since Sleeper has no webhooks. `linkSleeperDraft` requires the linked Sleeper draft's own `type` to match this season's `resolveDraftType`, and (snake/linear only) requires `drafts.draftOrder` to already be set to mirror Sleeper's real order — `applySleeperSyncTick` resolves each pick's `round`/`pickInRound`/`overallPick` via the same `resolveTeamPositionInRound`/`countRealSlotsThroughRound` helpers `draftPick`/`addKeeper` use (see [SNAKE.md](SNAKE.md)), rather than trusting Sleeper's own slot numbering, so a synced pick's slot always agrees with the board. `applySleeperSyncedPicks` itself branches on `price` vs `round` presence per pick, same pattern as `addKeeper`. UI: `SeasonSettingsTab.tsx`'s "Live sync from Sleeper" card (enable/disable, requires every team mapped to a Sleeper roster first) and a one-line status readout in `DraftTab.tsx`/`SnakeDraftTab.tsx`.
