...
export function AuthPanel() {
  ...
  return (
    <Stack gap="sm" py="sm">
      ...
      <Text c="dimmed">
        This uses Convex auth for your account. Google auth is not enabled in the current setup,
        but the auth framework is now in place for future provider expansion.
      </Text>
      {mode === 'signUp' && (
        <TextInput
          label="Name"
      ...
      <Group>
        ...
        <Button variant="default" onClick={() => setMode(mode === 'signIn' ? 'signUp' : 'signIn')}>
          {mode === 'signIn' ? 'Create account' : 'Use existing account'}
        </Button>
      </Group>
      {status && (
        <Alert color={status.kind === 'success' ? 'green' : 'red'} variant="light">
          {status.message}
        </Alert>
      )}
    </Stack>
